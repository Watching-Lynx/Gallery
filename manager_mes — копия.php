...
if(isset($_POST['subm_b']))
{
    ...
    $_SESSION['whosell'] = $tekuser['ФИО'];                                           //добавила я
                                    
    $colmes->updateOne(['Номер'=>$tekmes['Номер']],['$set' => ['Тип' => "Принято" ]]);
    header("Refresh:0");
    $_SESSION['sellnum'] = $tekmes['Номер']; 
...
</form><form id = 'dog' action="договорПродажи.php" target="_blank">
   <button>Переход по ссылке</button></form>
<table><td><a style = "text-decoration:none; " href="http://gallery/лк_manager.php">Вернуться</a></td></table></div>
  <!-- <script>
        document.getElementById('dog').style.display="none";  // for hide button
