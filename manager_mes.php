...
<div class = "mainblock"><div >Написано:</div> 
...
if($tekmes['Тип']=='Запрос')
{
  ...
  if($colmes->findOne(['Номер'=>$tekmes['Номер'],'Номер картины' => array('$exists' => true)]))
{
...
{
    if($colmes->findOne(['Номер'=>$tekmes['Номер'],"Номер картины" => array('$exists' => true)])){} else {
...
$_SESSION['dognum'] = $tekmes['Номер'];
$_SESSION['type'] =$tekmes['Тип'];

?>

</form>
<table><td><a style = "text-decoration:none; " href="http://gallery/лк_manager.php">Вернуться</a></td></table></form>
<br><form id = 'dog' action="договорПродажи.php" target="_blank">
   <button>Договор продажи</button></form><form id = 'dog' action="договорАренды.php" target="_blank">
   <button>Договор аренды</button></form></div>
